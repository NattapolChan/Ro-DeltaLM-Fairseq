...
from attention.alibi_attention import ALiBiMultiHeadAttention
...
def upgrade_state_dict_for_deltalm(
    ...
    for key in deltalm_state_dict.keys():
        ...
        if 'q_proj.bias' in key or 'v_proj.bias' or 'k_proj.bias':
            continue
...
class DeltaLMEncoder(TransformerEncoderBase):
    ...
    def build_encoder_layer(self, args, no_encoder_attn=False):
        layer = DeltaLMEncoderLayer(args, no_encoder_attn)
        if getattr(args, "checkpoint_activations", False):
            layer = checkpoint_wrapper(layer)
        return layer
    
class DeltaLMEncoderLayer(TransformerEncoderLayer):
    def __init__(
