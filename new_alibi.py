...
class LinearBiasedMultiheadAttention(nn.Module):

    # alibi masked can be shared across all MultiheadAttention to save more memory
    # later #
    ...
    def __init__(
        ...
        self.relative_position = torch.abs(relative_position).unsqueeze(0).expand(attn_heads, -1,-1)
        self.slopes = torch.Tensor(self.get_slopes(attn_heads)).cuda()*-1
    ...
    def forward(
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            ...
            print(self.alibi[:,:,:tgt_len, :src_len])
