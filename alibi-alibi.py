...
from attention.new_alibi import LinearBiasedMultiheadAttention 
...
class DeltaLMEncoder(TransformerEncoderBase):
    ...
    def build_encoder_layer(self, args):
        layer = DeltaLMEncoderLayer(args)
        if getattr(args, "checkpoint_activations", False):
            layer = checkpoint_wrapper(layer)
        return layer

class DeltaLMEncoderLayer(TransformerEncoderLayerBase):
    def __init__(self, args):
        super().__init__(args)
        print("init encoder layer base")
        self.args = args
    
    def build_self_attention(self, embed_dim, cfg):
        return LinearBiasedMultiheadAttention(
            embed_dim,
...
class DeltaLMDecoder(TransformerDecoderBase):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.alibi = self.init_alibi_mask(maxpos) * self.slopes.unsqueeze(1).unsqueeze(1)
        self.alibi = self.alibi.repeat(800, 1, 1)

    def init_alibi_mask(self, n: int):
        row_linear = torch.arange(n).unsqueeze(1)
        col_linear = torch.arange(n).unsqueeze(0)
        return row_linear - col_linear 
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (
            ...
            self._future_mask = self._future_mask - self.alibi

        self._future_mask = self._future_mask.to(tensor)
        return self._future_mask[:self.args.decoder_attention_heads * b, :dim, :dim]
